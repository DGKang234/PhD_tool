...
class AIMS:
    ...
    def Prepare_con_sub_files(self, FINAL_PATH_FULL, MOD_XYZ_LABEL):
        control = 'control.in.single' #'control.in.single'
        job_sub = 'trash_1.sh'
...
#gp_file=%s/GAP.xml' % (path, path))
        ...
        # Plotting pairwise interaction energy as a function of interatomic distances
    ...
    def Breathing(self, PATH):
        Breathing_dir = []
        with open(f'{PATH}/geometry.in', 'r') as f:
            lines = f.readlines()
            if '#' in lines[0]:
                del lines[0]

        coord = [x.split() for x in lines]
        atom_label = [x[-1] for x in coord]
        atom_marker = [x[0] for x in coord]
        coord = [x[1:-1] for x in coord]

        atom_label = np.asarray(atom_label)
        atom_marker = np.asarray(atom_marker)
        coord = np.asarray(coord).astype(float)

        # centre of masss
        com = coord.sum(axis=0)
        com = com/np.shape(coord)[0]
        coord_x = np.subtract(coord[:, 0], com[0], out=coord[:, 0])    # subtract all of x coordinate with x coordinate of com. same for all y, z
        ...
        for i in range(-100, 101, 10):
            breathing_coord = coord + (int(i)/100)
            breathing_coord = np.around((breathing_coord), 6)
            print(int(i)/100)
            stack = np.c_[atom_marker, breathing_coord, atom_label]
            stack = stack.tolist()
            print(stack)
            print()
             
            breathing_dir = PATH + '/' + 'B_' + str(int(i)/100)
            os.mkdir(breathing_dir)
            Breathing_dir.append(breathing_dir)
            with open(f'{breathing_dir}/geometry.in', 'w') as f:
                for j in stack:
                    for k in j:
                        new_line = k.ljust(15)
                        f.write(new_line)
                    f.write('\n')
        return Breathing_dir 
