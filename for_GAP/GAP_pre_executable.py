...
Brief explanation of what this code does:
    ...
    Then, do single point calculations using FHI-aims and generate extended XYZ file which includes forces of each atoms.
...
[gap_pre] directory contains all outputs
    inside of [gap_pre] there will be the directories are named with IP rank of the structure, [{IP rank}]
...
for f in files:                                                
    ...
    if FROM_rank <= marker <= TO_rank:
        ...
        for i in sub_wd:
            ...
            mod_list_PATH = sorted(mod_list_PATH, key=lambda x: x.split('/')[-1].split('_')[1].split('.')[0])
            ...
            for j in mod_list_PATH:
                ...
                if len(mod_dir_list) == 0:
                    ...
                    try:
                        ...
                        print(f'!!! [GREP] Error caused from: {j.split("gap_pre")[1]}  --- CHECK !!!')
                    except UnboundLocalError:
                        print(f'!!! [GREP] Error caused from: {j.split("gap_pre")[1]}  --- CHECK !!!')
                    except ValueError:
                        dummy_2.append(FINAL_PATH_FULL.split('/n4_rq')[-1])
                        print(f'!!! [EXT] Error caused from: {j.split("gap_pre")[1]}  --- CHECK !!!')


                    try:
                        GULP.Ext_xyz_gulp(FINAL_PATH_FULL, no_of_atoms, eigv_array, FORCES_GULP, tot_energy)
                        dummy_1.append(FINAL_PATH_FULL.split('/n4_rq')[-1])
